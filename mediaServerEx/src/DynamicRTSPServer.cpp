...
::lookupServerMediaSession(char const* fileName, Boolean isFirstLookupInSession) 
{
	// get filepath of exe and append fileName
	char filePathName[MAX_PATH + 1];
	//strcpy_s(filePathName, ExePath().c_str());
	//strcat_s(filePathName, fileName);
	strcpy_s(filePathName, fileName);

	FILE* fid = fopen(filePathName, "rb");
	Boolean fileExists = fid != NULL;

	// Next, check whether we already have a "ServerMediaSession" for this file:
	ServerMediaSession* sms = RTSPServer::lookupServerMediaSession(filePathName);
	...
	if (!fileExists) {
		...
		if (sms == NULL) {
			sms = createNewSMS(envir(), filePathName, fid);
